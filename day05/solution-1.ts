...
const executeMoves = (state: State): void => {
  for (const move of state.moves) {
    _.times(move.numCrates, () => {
      const content = state.stacks[move.from].pop();
      if (content === undefined) throw new Error('Tried to pop off empty stack');
      state.stacks[move.to].push(content);
    });
  }
};
